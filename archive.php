...
class redirector_reporting_class_archive {
	...
	function create_view() {
		...
		$sql = "CREATE OR REPLACE VIEW `{$wpdb->prefix}redirection_logs_view` AS 
			SELECT id, created, url, sent_to,agent, referrer,  redirection_id, ip, null as module_id, null as group_id FROM `{$wpdb->prefix}redirection_logs`
			UNION ALL
			SELECT id, created, url, sent_to, agent, referrer, redirection_id, ip, module_id, group_id FROM `{$wpdb->prefix}redirection_logs_archive`";
